...
//            wp_enqueue_script( 'at-multiselect-select2-js', $plugin_path . '/js/select2/select2.js', array( 'jquery' ), false, true );
        ...
        <span class="re-control"><img src="' . $plugin_path . '/images/remove.png" alt="' . esc_html( 'Remove', 'ifeature_cc_core' ) . '" title="' . esc_html( 'Remove', 'ifeature_cc_core' ) . '" id="remove-' . $field['id'] . '"></span>
        <span class="re-control-clear"></span></div>';
			...
			echo '" alt="' . esc_html( 'Add', 'ifeature_cc_core' ) . '" title="' . esc_html( 'Add', 'ifeature_cc_core' ) . '" id="add-' . $field['id'] . '"><br/></div>';
			...
			echo '</table><img src="' . $plugin_path . '/images/remove.png" alt="' . esc_html( 'Remove', 'ifeature_cc_core' ) . '" title="' . esc_html( 'Remove', 'ifeature_cc_core' ) . '" id="remove-' . $field['id'] . '"></div>';
...
.at-repater-block{background-color: #FFFFFF;border: 1px solid;margin: 2px; min-height: 50px}
   ...
   */
		...
		function Validate_upload_file_type( $file ) {
			if( isset( $_POST['uploadeType'] ) && !empty( $_POST['uploadeType'] ) && isset( $_POST['uploadeType'] ) && $_POST['uploadeType'] == 'my_meta_box' ) {
				...
				if( !in_array( $ext, (array)$allowed ) ) {
					$file['error'] = __( 'Sorry, you cannot upload this file type for this field.', 'ifeature' );
		...
		public function load_textdomain() {
			...
			load_textdomain( 'ifeature_cc_core', dirname( __FILE__ ) . '/lang/' . get_locale() . 'mo' );
		}
	} // End Class
endif; // End Check Class Exists

class Ifeature_Meta_Box extends AT_Meta_Box {
	...
	public function show_field_single_image( $field, $meta ) {
		...
		echo "<input type='button' class='upload-image-button' value='" . esc_html( 'Upload', 'ifeature_cc_core' ) . "' />";
		echo "<div class='upload-desc-field'>" . esc_html( 'or enter URL', 'ifeature_cc_core' ) . "</div>";
